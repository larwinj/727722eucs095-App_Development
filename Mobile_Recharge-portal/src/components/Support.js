...
const SupportCard = ({ location, address, phone, fax, customerCare }) => {
  return (
    <div className="support-card">
      <h2><span className="contact-label1">{location}</span></h2>
      <p className="address">{address}</p>
      {phone && <p><span className="contact-label">Phone:</span> {phone}</p>}
      {fax && <p><span className="contact-label">Fax:</span> {fax}</p>}
      {customerCare && <p><span className="contact-label">Customer Care:</span> {customerCare}</p>}
      <p><span className="contact-label">Email: </span>payeasecare@gmail.com</p>
    </div>
  );
};
  ...
  return (
    <div>
      ...
      <div className="support-page">
        <img src={spimg} alt="support image" className="support-img coll"/>
